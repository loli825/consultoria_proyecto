...
Observamos que la deviance es menor que la del modelo nulo, lo que nos esta indicando que al introducir las variables explicativas tenemos una mejora del modelo. Las covariables introducidas aportan información para estimar la probabilidad. El pseudo-$R^2$ bajo nos indica una explicación modesta de la variabilidad de los datos frente al modelo nulo. En un caso como el nuestro donde la variable respuesta esta desbalanceada (aproximadamente un 12% de éxito) esta métrica de ajsute global no es suficiente para decidir la correcta clasificación del modelo. Dada la baja prevalencia del evento, en términos de versosimilitudes ya se interpreta que un modelo nulo ajusta "correctamente" el evento de interés por la baja probabilidad que le asigna, por lo que un pequeño aumento obtervado en $pseudo-R^2$ puede no reflejar el potencial real de clasificación del modelo. Para ello emplearemos el area bajo la curva AUC que sí es un buen indicador de capacidad predictiva.
...
Podemos ver cómo la distancia de cook nos informa de uno o varios puntos realmente influyentes, por lo que vamos a proceder a detectarlos y eliminarlos para ver cómo cambia el mdoelo
...
En análisis del modelo muestra que la mayoría de las observaciones presentan una influencia mínima sobre el ajuste del modelo. No obstante, se identifican algunas observaciones con valores de Cook superiores al umbral, lo que indica una influencia potencialmente relevante. 
Al eliminar la observación influyente, el coeficiente de la interacción entre tamaño grande y soporte mural no puede ser estimado, por lo que teniamos muy pocas observaciones del efecto. 
...
Los valores psudo-$R^2$ son muy bajos, esto puede deberse, de nuevo, a que los datos estan desbalanceados y pueden ser ruidosos. Posteriormente, mediante la capacidad discriminativa acabamos de detectar si hay suficiente capacidad explicativa para determinar que hay diferencia respeto el modelo nulo.
...
## INTERPRETACIÓN RESULTADOS MODELO PRINCIPAL
...
Una tabla resusmen de lo visto anteriormente quedaria:
```{r tabla gt}
library(emmeans)
library(dplyr)
