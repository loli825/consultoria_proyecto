...
- Abril Penélope Diaz 1673688
- Angel Casado 1630811
...
```
Observamos que la deviance es menor que la del modelo nulo, lo que nos está indicando que al introducir las variables explicativas tenemos una mejora del modelo. Las covariables introducidas aportan información para estimar la probabilidad. El pseudo-$R^2$ bajo nos indica una explicación modesta de la variabilidad de los datos frente al modelo nulo. En un caso como el nuestro donde la variable respuesta está desbalanceada (aproximadamente un 12% de éxito), esta métrica de ajuste global no es suficiente para decidir la correcta clasificación del modelo. Dada la baja prevalencia del evento, en términos de verosimilitudes ya se interpreta que un modelo nulo ajusta "correctamente" el evento de interés por la baja probabilidad que le asigna, por lo que un pequeño aumento observado en $pseudo\text{-}R^2$ puede no reflejar el potencial real de clasificación del modelo. Para ello emplearemos el área bajo la curva AUC, que sí es un buen indicador de capacidad predictiva.
...
El gráfico de la curva ROC del modelo principal se sitúa por encima de la diagonal de referencia, lo que nos sugiere que el modelo es adecuado.
...
Los residuos de Pearson se encuentran en torno a 0, lo que indica la ausencia de sesgo sistemático en las predicciones.
...
Podemos ver cómo la distancia de Cook nos informa de uno o varios puntos realmente influyentes, por lo que vamos a proceder a detectarlos y eliminarlos para ver cómo cambia el modelo.

```{r, echo=F}
cook <- cooks.distance(m_principal)
i_max <- which.max(cook)

df2 <- df[-i_max, , drop=FALSE]
m_principal2 <- update(m_principal, data = df2)

b1 <- coef(m_principal)
b2 <- coef(m_principal2)  # el modelo sin el influyente
cmp <- cbind(b_con = b1, b_sin = b2, diff = b2 - b1, rel = (b2 - b1)/abs(b1))
cmp[order(abs(cmp[,"diff"]), decreasing = TRUE), ]

m_principal <- m_principal2

# Tabla de la interacción respecto a exito
with(df2, table(soporte_grp, tam_cat, exito))

cook <- cooks.distance(m_principal2)
plot(cook, type = "h",
     main = "Distancia de Cook – Modelo principal",
     ylab = "Cook's distance")
abline(h = 4 / length(cook), lty = 2, col = "red")
...
Una vez detectado el punto influyente, creamos un nuevo modelo eliminando dicho punto para detectar si realmente tenemos otros puntos influyentes.
El análisis del modelo muestra que la mayoría de las observaciones presentan una influencia mínima sobre el ajuste del modelo. No obstante, se identifican algunas observaciones con valores de Cook superiores al umbral, lo que indica una influencia potencialmente relevante.
Al eliminar la observación influyente, el coeficiente de la interacción entre tamaño grande y soporte mural no puede ser estimado, por lo que teníamos muy pocas observaciones para estimar el efecto.

Finalmente, se observa que algunas interacciones presentan tamaños muestrales muy bajos (n = 1, 2 o 3), lo que da lugar a situaciones en las que la variable de éxito o fracaso es 0, lo que crea errores en la predicción.

Cabe recalcar que al eliminar el punto influyente y ver las posibles combinaciones vemos que encontramos celdas vacias, salen Na's al eliminarla. Por lo tanto al ser un problema que no sabemos como tratar mantendremos el análisis de la misma forma con ese valor influyente para evitar problemas mayores. Fuera de este valor influyente vemos que el resto de datos pueden ser tratados ya que no tienen una grn influencia.
**Multicoleinealidad**

Como hemos comentado anteriormente, los valores de GVIF altos en las variables soporte_grp y en la interacción pueden deberse a sus grados de libertad, ya que en el GVIF corregido no presentan valores elevados. Por tanto, el modelo no presenta problemas de multicolinealidad.

El modelo m_principal se considera adecuado y fiable para predecir la probabilidad de éxito.

**Validación de modelos alternativos**
En esta sección se proponen estructuras de modelos que fueron descartadas como opciones al modelo principal, pero que por interés interpretativo se ajustan igualmente. Primero presentaremos las propuestas y después serán reestimadas utilizando reducción de sesgo de Firth cuando sea necesario.

- Modelo simplificado

Dada la complejidad y la posible separación debida a la interacción «soporte_grp × tam_cat», aunque ya se controló adecuadamente, se ha decidido estudiar también la posibilidad de un modelo simplificado sin interacciones:
...
- Variable tema
Dado el interés interpretativo del tema, y visto que sí aporta información adicional pero incrementa fuertemente la complejidad (muchos niveles), su inclusión se analiza en esta sección de modelos alternativos siguiendo las siguientes estrategias:
...
3) Adicionalmente, una versión parsimoniosa usando log(área) lineal para cada caso anterior

De esta manera se explorará el efecto del tema tanto controlando las covariables de técnica/soporte/montaje como sin ellas para, de esta manera, poder obtener también una estimación global de su asociación con la respuesta. Esta decisión viene dada como consecuencia de nuestros objetivos e hipótesis principales. En un inicio se planteó la posibilidad de que la iconografía de la pintura pudiera estar relacionada con el cumplimiento de la razón áurea; por esta razón queremos explorar el efecto global de esta variable "tema", aun sabiendo que el efecto de técnica y soporte podría estar incluido en la relación, ya que este modelo no pretende estimar un efecto causal directo, sino capturar la asociación total que puede incluir diferencias mediadas por técnica/soporte. Por otra parte, con el objetivo de no sesgar totalmente la interpretación, también se estudiará el efecto en conjunto con estas variables: "soporte_grp" y "técnica", las cuales también eran de interés. Finalmente, se contempla la posibilidad de cambiar la especificación del tamaño por su versión continua lineal (log(área)) con el objetivo de simplemente controlar el efecto del tamaño, pero sin interés interpretativo en esta sección.
...
```
Para comenzar, podemos ver que tema siempre resulta significativo en cuanto aporta información, es decir, independientemente de cómo controlemos el tamaño y de si incluimos o no el bloque 3 (técnica/soporte/montaje), tema añade información. Este resultado avala nuestra intención de realizar esta sección para interpretar la asociación de la iconografía con el cumplimiento de la razón áurea.

- Comparación de modelos
Para decidir qué modelo o modelos interpretar utilizaremos los criterios AIC/BIC. Se muestran sus valores para cada modelo y una tabla de incrementos, de manera que resulte legible.
...
```
Dado que pretendemos explorar la asociación de tema tanto controlando por las covariables del bloque 3 como sin ellas, seleccionaremos de cada una la opción que proporcione mejores valores de AIC/BIC.
...
Podemos ver que en ambos casos, con la inclusión del bloque 3 (técnica/soporte/montaje) o sin ella, los modelos preferibles son aquellos con una especificación categórica del tamaño, es decir, utilizando "tam_cat" en vez de "area". Por esta razón queda eliminada la opción alternativa que se había planteado en relación con el tamaño.

Los modelos finales que se utilizarán para interpretar la asociación de "tema" son 'mT_full_tam' y 'mT_noB3_tam'.
...
Para evaluar el posible ajuste ediante Firth, estudiaremos la separación de los 3 modelos:
...
Los valores reducidos del psudo-$R^2$ nos indican que los modelos mejoran al modelo nulo, lo que sugiere que los modelos tienen una capacidad explicativa modesta.

Los valores psudo-$R^2$ son muy bajos, esto puede deberse, de nuevo, a que los datos estan desbalanceados y pueden ser ruidosos. Posteriormente, mediante la capacidad discriminativa acabamos de detectar si hay suficiente capacidad explicativa para determinar que hay diferencia respecto el modelo nulo.
...
El AUC de los tres modelos alternativos muestran discriminación moderada de los datos.
...
Los residuos no muestran patrones ni grandes desviaciones respecto al 0, el análisi de los residuos nos sugiere que los modelos están bien ajustados.
...
lapply(modelos, vif)
```
Analizamos la multicolinealidad mediante el VIF y GVIF, los cuales no muestran valores elevados en ninguno de los predictores de ningún modelo. Esto nos indica que los modelos alternativos no presentan problemas de multicolinealidad.

# 6. Análisis principal
En esta sección presentamos los resultados correspondientes al modelo escogido (m_principal), haremos una interpretación de estos modelos a partir de los efectos marginales; de esta manera podremos analizar el efecto de cada variable cuando mantenemos las demás constantes. También haremos comparaciones a pares para ver la diferencia entre los niveles específicos de cada variable y ver si estas diferencias son estadísticamente significativas o si no podemos confirmar que lo sean.



## 6.1 Resultados modelo principal

```{r, echo=F}
summary(m_principal)
```

**Efectos marginales de variables categòricas**
...
pairs(emm_tecnica)
```
Vemos que los efectos de la tecnica no son significativos y no encontramos diferencias significativas entre grupos.


Al final del documento encontramos una tabla resumen de toda esta información unificada:
```{r tabla gt, echo=F}
library(emmeans)
library(dplyr)

pairs_principal <- bind_rows(

  # Tamaño condicionado al soporte
  pairs(emmeans(m_principal, ~ tam_cat | soporte_grp)) %>%
    as.data.frame() %>%
    mutate(
      Variable = paste0(
        "Tamaño (soporte = ", soporte_grp, ")"
      )
    ),

  # Soporte condicionado al tamaño
  pairs(emmeans(m_principal, ~ soporte_grp | tam_cat)) %>%
    as.data.frame() %>%
    mutate(
      Variable = paste0(
        "Soporte (tamaño = ", tam_cat, ")"
      )
    ),

  # Variables sin interacción
  pairs(emmeans(m_principal, ~ orientacion)) %>%
    as.data.frame() %>%
    mutate(Variable = "Orientación"),

  pairs(emmeans(m_principal, ~ sop_montaje)) %>%
    as.data.frame() %>%
    mutate(Variable = "Soporte de montaje"),

  pairs(emmeans(m_principal, ~ serie)) %>%
    as.data.frame() %>%
    mutate(Variable = "Serie")
)

tabla_pairs_principal <- pairs_principal %>%
  transmute(
    Variable,
...
tabla_pairs_principal %>%
  gt() %>%
  tab_header(
    title = "Efectos marginales comparados modelo principal (odds ratios)",
    subtitle = "Comparaciones por pares – modelo con interacción tamaño × soporte"
  ) %>%
  cols_label(
    Variable = "Variable",
...
print(tabla_pairs_principal)
```

## 7. Modelos alternativos
En esta sección presentamos los resultados correspondientes a los modelos alternativos mA1, mA2 y mA3. Generalmente haremos una interpretación de estos modelos a partir de los efectos marginales; de esta manera podremos analizar el efecto de cada variable cuando mantenemos las demás constantes. También haremos comparaciones a pares para ver la diferencia entre los niveles específicos de cada variable y ver si estas diferencias son estadísticamente significativas o si no podemos confirmar que lo sean.
...
print(tabla_pairs_mA1)
```
**LA TABLA SE IMPRIME AL FINAL DEL DOCUMENTO CON EL NOMBRE Efectos marginales comparados (odds ratios) Comparaciones por pares –modelo mA1, CONSULTE AHI LA INFORMACIÓN**
...
tabla_pairs_mA2 %>%
  ...
  opt_table_outline() %>%
  opt_row_striping()
print(tabla_pairs_mA2)
```

**LA TABLA SE IMPRIME AL FINAL DEL DOCUMENTO CON EL NOMBRE Efectos marginales comparados (odds ratios) Comparaciones por pares –modelo mA2, CONSULTE AHI LA INFORMACIÓN**
...
print(tabla_pairs_mA3)
```
**LA TABLA SE IMPRIME AL FINAL DEL DOCUMENTO CON EL NOMBRE Efectos marginales comparados (odds ratios) Comparaciones por pares –modelo mA3, CONSULTE AHI LA INFORMACIÓN**
...
## 7.1 Análisis secundarios

Además del objetivo principal del estudio, la base de datos obtenida permite explorar de forma complementaria la capacidad explicativa de otras variables de interés. Una de las que más interés genera es la fecha estimada del cuadro, ya que hemos tenido también que obtener la variable fecha_ancho, dado que algunas obras presentan incertidumbre.

Para ello, ajustamos un modelo de regresión lineal incluyendo como variables explicativas el tema, el tipo de soporte, el tamaño en categorías, la orientación, el tipo de autor, la técnica, la serie y la interacción de serie con tamaño.
...
Los resultados nos indican que, para predecir la fecha estimada del cuadro, todas las variables seleccionadas son estadísticamente significativas, incluyendo la interacción. Esto sugiere que la evolución temporal de las obras depende de la mayoría de las características del cuadro que tenemos en la base de datos y de la combinación de la serie con el tamaño.


# 8. CONCLUSIONES
## 8.1 Descubrimientos del estudio
A partir del análisis realizado, se observa que el cumplimiento de la razón áurea en las pinturas del Museo del Prado no responde a un patrón único , sino que está condicionado por determinados factores tanto históricos como de forma de las mismas pinturas. En relación con la primera hipótesis, los resultados muestran una asociación clara entre la fecha de creación de la obra y la probabilidad de cumplir la razón áurea, siendo las obras más recientes las que presentan una mayor frecuencia de cumplimiento. Hemos detectado posibles problemas en estos resultados que comentaremos más adelante.

Respecto al tamaño de la obra, los resultados apoyan parcialmente la segunda hipótesis. Si bien el tamaño por sí solo no explica de forma uniforme el cumplimiento de la razón áurea, las obras de gran formato presentan mayores probabilidades de cumplirla, aunque estos resultados también son favorecidos en obras de tamaño pequeño, mientras que viendo que las de tamaño mediano son las que menos probabilidades tienen de cumplir la proporción áurea. Por lo que vemos que el tamaño está relacionado con el cumplimiento de esta pero no sigue una tendencia lineal en cuanto al mismo.

En relación con la tercera hipótesis, el análisis muestra que el efecto del tamaño depende del tipo de soporte utilizado. Esto sugiere que el material de soporte actúa como un factor condicionante de las dimensiones posibles de la obra y, en consecuencia, de las decisiones compositivas asociadas a la proporción áurea. En algunos soportes, el tamaño parece ser un elemento clave, mientras que en otros la relación es menos clara, posiblemente debido a limitaciones físicas o funcionales propias del material.

Por el contrario, la iconografía de la pintura y las técnicas empleadas no muestran una relación sistemática con el cumplimiento de la razón áurea, lo que lleva a rechazar o las hipótesis cuarta y quinta. Aunque estos elementos forman parte del contexto artístico y pueden influir en la composición interna de la obra, los resultados sugieren que su efecto sobre la proporción global es indirecto. Vemos que en tema si que hay una comparación que da resultados significativos en la diferencia, pero siendo una de muchas no podemos concluir que sea un efecto significativo.

En conjunto, los resultados muestran que las variables que influyen en el cunplimiento de la razón aurea son: el tamaño, si pertenece a serie, la orientación del cuadro y el tipo de soporte del cuadro. También la fecha del cuadro con las limitaciones de las que hablaremos a continuación.

## 8.2 Continuidad del proyecto: Limitaciones y propuestas
**LIMITACIONES**
En primer lugar, una limitación relevante del estudio deriva de la naturaleza de la base de datos utilizada. El análisis se basa exclusivamente en pinturas que están disponibles y catalogadas en la base de datos del Museo del Prado, lo que implica un posible sesgo de selección. Es razonable pensar que las obras conservadas, expuestas o suficientemente documentadas no constituyen una muestra aleatoria del conjunto de la producción pictórica histórica. Pinturas menos conocidas, deterioradas, perdidas o no incorporadas al catálogo podrían presentar características compositivas distintas, lo que limita la generalización de los resultados más allá del ámbito del museo y de las obras que han llegado hasta nuestros días.

En segundo lugar, el modelo plantea una simplificación en la relación entre algunas variables continuas y la probabilidad de cumplimiento de la proporción áurea. Aunque se introducen transformaciones y funciones suaves (como en el caso de la fecha estimada), el enfoque sigue imponiendo una estructura relativamente parsimoniosa. Es posible que existan relaciones no lineales más complejas, es posible que relaciones no queden completamente capturadas por el modelo especificado. Esta limitación es especialmente relevante en un contexto histórico, donde los cambios estilísticos no siempre se producen de manera gradual.

Por último, el modelo logístico utilizado asume independencia entre las observaciones, una hipótesis que puede no cumplirse estrictamente en este conjunto de datos. Varias pinturas pueden pertenecer a una misma serie, compartir autor, taller o contexto de producción, lo que introduce una posible correlación no modelada entre obras. Ignorar esta estructura jerárquica puede llevar a una subestimación de la incertidumbre asociada a algunos efectos y a una sobreinterpretación de su significación estadística. Abordar esta cuestión requeriría modelos más complejos, como modelos mixtos con efectos aleatorios por autor o serie, que quedan fuera del alcance de este análisis.


**PROPUESTAS** 
Una primera línea de trabajo futuro consistiría en analizar la heterogeneidad de los determinantes del éxito según el contexto histórico y cultural. En particular, resultaría interesante estudiar si los factores asociados al cumplimiento de la proporción áurea varían entre siglos, corrientes artísticas o países de origen. Este enfoque permitiría evaluar si los resultados obtenidos responden a patrones generales o si, por el contrario, la importancia de variables como el tamaño, el soporte o la técnica depende del marco estilístico e histórico en el que se produce la obra. Metodológicamente, esto podría abordarse mediante modelos estratificados o mediante interacciones explícitas entre las variables compositivas y los indicadores de periodo o escuela artística.

En segundo lugar, una extensión natural del trabajo sería evaluar la capacidad predictiva del modelo en un conjunto de datos independiente. Utilizar otra base de datos, ya sea de un museo distinto o una actualización más reciente del catálogo, permitiría comprobar hasta qué punto las relaciones estimadas son robustas y no específicas de la muestra analizada. Este ejercicio reforzaría la validez externa del estudio y ayudaría a distinguir entre patrones estructurales y resultados condicionados por el proceso de selección de las obras disponibles.

Por último, el análisis podría enriquecerse incorporando características propias de los autores, con el objetivo de estudiar si existen factores exclusivamente ligados al pintor que influyen en la probabilidad de cumplimiento de la proporción áurea. Variables como la formación, la pertenencia a un taller, la etapa vital del autor o su  escuela concreta podrían aportar información adicional. Este enfoque permitiría separar, al menos parcialmente, los efectos atribuibles a las características materiales de la obra de aquellos asociados al estilo y trayectoria del artista.
