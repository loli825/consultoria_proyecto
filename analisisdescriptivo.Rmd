...
```{r}
library(readr)
df <- read_csv("consultoria_proyecto/prado_variables.csv")
```
...
# matriz: filas técnicas, columnas exito (proporciones dentro de cada técnica)
mat <- sapply(tec_cols, function(v) {
  idx <- df[[v]] == 1
  prop.table(table(df$exito_f[idx]))
})

mat <- t(mat)               # técnicas en filas
mat[is.na(mat)] <- 0

# colores fijos por categoría de éxito
cols <- c("red", "green")   # No cumple / Sí cumple
...
barplot(t(mat),
        ...
        las = 2,
        col = cols,
        main = "Distribución de éxito entre obras con cada técnica",
        ylab = "Proporción")
